...
export default async function VehicleDetailPage({
  ...
  return (
    <main className="space-y-5 px-4 pb-28 pt-4">
      ...
      <SellerCard name={vehicle.sellerName} role={vehicle.sellerRole} phone={vehicle.sellerPhone} />
      ...
      <div className="fixed inset-x-0 bottom-16 z-30 mx-auto flex w-full max-w-xl gap-2 border-t border-slate-100 bg-white p-3">
        <CallButton phone={vehicle.sellerPhone} text="Call Seller" className="flex-1" />
        <Link
          href={`https://wa.me/${vehicle.sellerPhone.replace(/[^\d+]/g, "")}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl bg-emerald-500 px-4 text-sm font-semibold text-white"
        >
          WhatsApp
        </Link>
      </div>
