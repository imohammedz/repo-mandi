...
export function InsuranceRenewalCard({ vehicleId, vehicleTitle, insuranceValidTill, alertType }: Props) {
  ...
  const submitInquiry = async (phoneVerified: boolean) => {
    ...
    const cleanedRequirement = requirementText.trim() || "Need insurance support for this vehicle.";

    const response = await fetch("/api/insurance-inquiries", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        requirementText: cleanedRequirement,
        phoneVerified,
      }),
    });
    ...
    setSuccessMessage("Insurance inquiry submitted. Our team will contact you shortly.");
    close();
  };
  ...
  return (
    <>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold ${alertToneClass}`}>
          <ShieldAlert className="h-3.5 w-3.5" />
          {alertType === "EXPIRED" ? "Insurance Expired" : "Insurance Expiring Soon"}
        </div>
        <h2 className="mt-3 text-base font-semibold text-slate-900">Get Insurance Support</h2>
        <p className="mt-2 text-sm text-slate-600">
          Insurance valid till {insuranceValidTill}. Submit your details and our insurance team will contact you.
        </p>
        <button
          onClick={() => {
            setError("");
      ...
      {open ? (
        <div className="fixed inset-0 z-50">
          <button
            aria-label="Close insurance inquiry sheet"
            onClick={close}
          ...
          <section className="absolute inset-x-0 bottom-0 rounded-t-3xl bg-white p-4 shadow-2xl">
            ...
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-900">Get Insurance</h3>
              <button
                onClick={close}
            ...
            {!awaitingOtp ? (
              <div className="space-y-3">
                ...
                <label className="block text-xs text-slate-500">
                  Insurance Requirement (Optional)
                  <textarea
                    value={requirementText}
                    onChange={(event) => setRequirementText(event.target.value)}
                    placeholder="Example: Need comprehensive cover with IDV details."
                    rows={3}
